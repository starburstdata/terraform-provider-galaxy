...
func (d *snowflake_catalog_validationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading snowflake_catalog_validation", map[string]interface{}{"catalog_id": id})

	response, err := d.client.ValidateCatalog(ctx, "snowflake", id)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error reading snowflake_catalog_validation",
			"Could not read snowflake_catalog_validation catalogId: "+err.Error(),
		)
		return
	}
