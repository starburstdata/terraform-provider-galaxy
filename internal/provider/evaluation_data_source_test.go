...
func TestAccDataSourceEvaluation_Basic(t *testing.T) {
	suffix := testSuffix
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceEvaluationConfig(suffix),
...
func testAccDataSourceEvaluationConfig(suffix string) string {
	return fmt.Sprintf(`
...
resource "galaxy_postgresql_catalog" "test" {
  ...
  read_only     = false
  description   = "PostgreSQL catalog for evaluation data source test"
}
...
resource "galaxy_role" "dq_check_grant" {
  role_name              = "evldsgrant%[1]s"
  role_description       = "Role granting query access to the evaluation data source test catalog"
...
resource "galaxy_data_quality_check" "test" {
  name        = "evalds_%[1]s"
  description = "Data quality check for evaluation data source test"
