...
func (d *cassandra_catalog_validationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	tflog.Debug(ctx, "Reading cassandra_catalog_validation", map[string]interface{}{"catalog_id": id})

	response, err := d.client.ValidateCatalog(ctx, "cassandra", id)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error reading cassandra_catalog_validation",
			"Could not read cassandra_catalog_validation catalogId: "+err.Error(),
		)
		return
	}
