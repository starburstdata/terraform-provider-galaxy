...
func TestAccDataSourceDataQualityChecks_List(t *testing.T) {
	suffix := testSuffix
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceDataQualityChecksConfig(suffix),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue(
						"data.galaxy_data_quality_checks.test",
...
func testAccDataSourceDataQualityChecksConfig(suffix string) string {
	return fmt.Sprintf(`
...
resource "galaxy_postgresql_catalog" "test" {
  ...
  read_only     = false
  description   = "PostgreSQL catalog for DQ checks list data source test"
}
...
resource "galaxy_role" "dq_check_grant" {
  role_name              = "dqcklsgrant%[1]s"
  role_description       = "Role granting query access to the DQ checks list data source test catalog"
...
resource "galaxy_data_quality_check" "test" {
  name        = "dqchkls_%[1]s"
  description = "Data quality check for list data source test"
