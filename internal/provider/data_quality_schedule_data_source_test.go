...
func TestAccDataSourceDataQualitySchedule_Basic(t *testing.T) {
	suffix := testSuffix
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceDataQualityScheduleConfig(suffix),
...
func testAccDataSourceDataQualityScheduleConfig(suffix string) string {
	return fmt.Sprintf(`
...
resource "galaxy_postgresql_catalog" "test" {
  ...
  read_only     = false
  description   = "PostgreSQL catalog for data quality schedule data source test"
}
...
resource "galaxy_role" "dq_check_grant" {
  role_name              = "dqschgrant%[1]s"
  role_description       = "Role granting query access to the data quality schedule data source test catalog"
...
resource "galaxy_data_quality_check" "test" {
  name        = "dqschd_%[1]s"
  description = "Data quality check to create schedule context"
