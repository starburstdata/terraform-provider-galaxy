...
var _ resource.ResourceWithImportState = (*sqlserver_catalogResource)(nil)
...
func (r *sqlserver_catalogResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	...
	if attr, ok := s.Attributes["endpoint"].(schema.StringAttribute); ok {
		attr.Description = "SQL Server database endpoint. At least one of endpoint or private_link_id must be specified."
...
func (r *sqlserver_catalogResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
...
func (r *sqlserver_catalogResource) modelToCreateRequest(ctx context.Context, model *resource_sqlserver_catalog.SqlserverCatalogModel, diags *diag.Diagnostics) map[string]interface{} {
	...
	if !hasEndpoint && !hasPrivateLinkId {
		diags.AddError(
			"Missing required field",
			"Either endpoint or private_link_id must be specified for sqlserver_catalog",
		)
		return request
	}
	if hasPrivateLinkId && hasSshTunnelId {
		diags.AddError(
			...
			"ssh_tunnel_id and private_link_id are mutually exclusive for sqlserver_catalog",
		)
		return request
	}
