...
package datasource_row_filters
...
func RowFiltersDataSourceSchema(ctx context.Context) schema.Schema {
...
func (t ResultType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	...
	rowFilterIdAttribute, ok := attributes["row_filter_id"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`row_filter_id is missing from object`)

		return nil, diags
	}

	rowFilterIdVal, ok := rowFilterIdAttribute.(basetypes.StringValue)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`row_filter_id expected to be basetypes.StringValue, was: %T`, rowFilterIdAttribute))
	}
...
type ResultValue struct {
	...
	RowFilterId basetypes.StringValue `tfsdk:"row_filter_id"`
...
func (v ResultValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	...
	attrTypes["row_filter_id"] = basetypes.StringType{}.TerraformType(ctx)
	...
	case attr.ValueStateKnown:
		...
		val, err = v.RowFilterId.ToTerraformValue(ctx)

		if err != nil {
			return tftypes.NewValue(objectType, tftypes.UnknownValue), err
		}

		vals["row_filter_id"] = val
...
func (v ResultValue) ToObjectValue(ctx context.Context) (basetypes.ObjectValue, diag.Diagnostics) {
	...
	attributeTypes := map[string]attr.Type{
		...
		"row_filter_id": basetypes.StringType{},
	}
...
func (v ResultValue) Equal(o attr.Value) bool {
	...
	if !v.RowFilterId.Equal(other.RowFilterId) {
		return false
	}
